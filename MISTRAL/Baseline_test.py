...
MODEL_BEING_TESTED = "mistralai/Mixtral-8x7B-Instruct-v0.1" # "mistralai/Mixtral-8x7B-Instruct-v0.1"
...
OUTPUT_CSV_FILE = "/app/results/results_vllm_mistral_baseline.csv"  # Save to mounted volume
...
def log_result(model_id, num_threads, test_type, loop_num,
            question, expected, output, exec_time, accuracy):
...
def test_worker(client, model_id, num_threads):
    try:
        truncated_haystack=""  # here we pass it empty for baseline test
        for i in range(1, SIMPLE_BASELINE_LOOPS + 1):
            for qa_pair in GENERAL_KNOWLEDGE_QUESTIONS:
                q, ans = qa_pair["question"], qa_pair["answer"]
                prompt = f"""Based on your general knowledge,{truncated_haystack} please answer the following question.\n\nQUESTION:\n{q}"""
        ...
        error_msg = f"OOM Error: {str(e)}"
        print(f"ERROR: {error_msg}")
        log_error(MODEL_BEING_TESTED, num_threads, "OOM", error_msg)
        raise

    except Exception as e:
        error_msg = f"Error: {str(e)}"
        print(f"ERROR: {error_msg}")
        log_error(MODEL_BEING_TESTED, num_threads, "Exception", error_msg)


def log_error(model_id, num_threads, error_type, error_msg):
    """Log error results with consistent structure"""
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    row = [
        timestamp, model_id, num_threads, "ERROR", error_type,
        error_msg, "N/A", "N/A", 0, "Fail"
    ]
    with csv_lock:
        with open(OUTPUT_CSV_FILE, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
...
def main():
    ...
    # --- SINGLE-THREAD TEST MODE ---
    num_threads = 1
    print(f"\n--- Running test with exactly {num_threads} thread ---")
    ...
    print(
        f"Peak GPU: {summary['peak_gpu']}% | Peak VRAM: {summary['peak_vram']:.2f}GB | Avg VRAM: {summary['avg_vram']:.2f}GB"
    )

    print(
        f"\n{'=' * 20} Finished single-thread test for model: {MODEL_BEING_TESTED} {'=' * 20}"
    )
